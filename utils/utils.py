...
import os, stat, sys, shutil, json, re, subprocess, errno, gzip
...
class Utils:
    ...
    @staticmethod
    def download(url, fnp, auth=None, force=None, file_size_bytes=0, skipSizeCheck=None,
                 quiet = False, umask=stat.S_IRUSR | stat.S_IWUSR | stat.S_IRGRP | stat.S_IWGRP):
        Utils.ensureDir(fnp)
    ...
    @staticmethod
    def ensureDir(fnp, umask=stat.S_IRUSR | stat.S_IWUSR | stat.S_IRGRP | stat.S_IWGRP):
    ...
    @staticmethod
    def mkdir(path, umask=stat.S_IRUSR | stat.S_IWUSR | stat.S_IRGRP | stat.S_IWGRP):
        try:
            os.mkdir(path)
            # chmod g+w
            st = os.stat(path)
    ...
    @staticmethod
    def mkdir_p(path, umask=stat.S_IRUSR | stat.S_IWUSR | stat.S_IRGRP | stat.S_IWGRP):
...
def main():
    url = "http://purcaro.com/asdflasldfas"
    try:
        print("normal")
        q = Utils.download(url, "/tmp/a")
    except:
        pass
    ...
    try:
        print("******************")
        print("quiet")
        q = Utils.download(url, "/tmp/a", quiet=True)
    except:
        pass
